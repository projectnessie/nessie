...
```shell
docker run --rm ghcr.io/projectnessie/nessie-gc:0.106.1 create-sql-schema \
...
```shell
docker run --rm ghcr.io/projectnessie/nessie-gc:0.106.1 gc \
  --jdbc-url jdbc:postgresql://127.0.0.1:5432/nessie_gc \
  --jdbc-user pguser \
  --jdbc-password mysecretpassword
```
...
ghcr.io/projectnessie/nessie-gc:0.106.1 --help`. The main command is `gc`,
which is followed by subcommands and options. Check the available subcommands and options by running
`docker run --rm ghcr.io/projectnessie/nessie-gc:0.106.1 gc --help`.
...
spec:
  template:
    spec:
      containers:
      - name: nessie-gc
        image: ghcr.io/projectnessie/nessie-gc:0.106.1
        args: 
          - gc
          - --uri
