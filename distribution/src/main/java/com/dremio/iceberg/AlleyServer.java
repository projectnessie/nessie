...
import com.dremio.iceberg.server.ServerConfigurationImpl;
...
public class AlleyServer implements Closeable {
  private static final org.slf4j.Logger logger =
    org.slf4j.LoggerFactory.getLogger(AlleyServer.class);
  ...
  public void start() throws Exception {
    ...
    if (config.getServiceConfiguration().metricsEnbled()) {
      metrics();
    }
    ...
    if (config.getServiceConfiguration().swaggerEnabled()) {
    ...
    if (config.getServiceConfiguration().uiEnabled()) {
  ...
  protected void addStaticPath(ServletHolder holder, String basePath,
                               String relativeMarkerPathToResource) throws URISyntaxException {
