...
public class ListTags {
  ...
  public Response setTable(Tag tag) {
    try {
      ...
      ImmutableTag.Builder newTag = ImmutableTag.builder().from(tag).updateTime(updateTime).id(id);
      if (tag.getBaseTag() == null) {
        newTag = createFromCurrentState(newTag);
      } else {
        newTag = createFromTag(newTag, tag.getBaseTag());
      }
  ...
  private ImmutableTag.Builder createFromCurrentState(ImmutableTag.Builder tag) {
    List<VersionedWrapper<Table>> tables = backend.tableBackend().getAll(false);
    Map<String, TableVersion> versions = tables.stream()
                                               .map(VersionedWrapper::getObj)
                                               .map(t -> t.getVersionList()
                                                          .get(t.getMetadataLocation()))
                                               .collect(Collectors.toMap(
                                                 TableVersion::getUuid,
                                                 Function.identity()
                                               ));
    return tag.tableVersions(versions);
  }

  private ImmutableTag.Builder createFromTag(ImmutableTag.Builder tag, String baseTag) {
    VersionedWrapper<Tag> parentVersionedTag = backend.tagBackend().get(baseTag);
    if (parentVersionedTag == null) {
      throw new NotFoundException();
    }
    Tag parentTag = parentVersionedTag.getObj();
    Map<String, TableVersion> snapshots = parentTag.getTableVersions()
                                                   .values()
                                                   .stream()
                                                   .collect(Collectors.toMap(TableVersion::getUuid,
                                                                             Function.identity()));
    return tag.tableVersions(snapshots);
  }
  ...
  private Response verifySnapshot(TableVersion tableVersion) {
    ...
    boolean found = table.getVersionList().values().stream()
  ...
  public Response deleteTable(@PathParam("name") String name,
    ...
    try {
      ...
      if (purge) {
        ...
        Tag newTag = ImmutableTag.builder().from(tag.getObj())
  ...
  public Response updateTable(@PathParam("name") String name, @Context Request request, Tag tag) {
    ...
    if (evaluationResultBuilder == null) {
      try {
        List<Response> checkResponse = tag.getTableVersions()
                                          .values()
                                          .stream()
        ...
        Tag newTag = ImmutableTag.builder().from(tag)
                        .updateTime(ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli())
                        .build();
        backend.tagBackend()
