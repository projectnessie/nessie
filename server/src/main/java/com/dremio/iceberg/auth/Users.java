...
public class Users {
  ...
  public Response getUsers(@PathParam("name") String name, @Context SecurityContext context) {
    ...
    if (user.isPresent()) {
      return Response.ok(
        Users.toUser(user.get())).tag(new EntityTag(user.get().unwrap().getVersion().toString())).build();
    } else {
  ...
  public Response addUser(User user) {
    try {
      Optional<com.dremio.iceberg.auth.User> userExisting = backend.fetchAll().stream()
        .filter(u -> u.getName().equals(user.getId()))
        .findFirst();
      if (userExisting.isPresent()) {
        return Response.status(409).build();
      }
      long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
      backend.create(Users.fromUser(user.withUpdateTime(updateTime)));
  ...
  public Response modifyUser(@PathParam("name") String name, User user, @Context SecurityContext context, @Context Request request) {
    ...
    EntityTag eTag = new EntityTag(oldUser.get().unwrap().getVersion().toString());
    ...
    try {
      ...
      long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
      backend.update(Users.fromUser(user.withUpdateTime(updateTime)));
