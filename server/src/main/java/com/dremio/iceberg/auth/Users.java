...
public class Users {
  ...
  public Response getUsers(@Context SecurityContext context) {
    if (context.isUserInRole("admin")) {
      List<User> users =
        backend.fetchAll().stream().map(Users::toUser).collect(Collectors.toList());
      return Response.ok(users.toArray(new User[]{})).build();
    } else {
      Optional<com.dremio.iceberg.auth.User> user =
        backend.fetch(context.getUserPrincipal().getName());
      if (!user.isPresent()) {
  ...
  public Response addUser(User user) {
    try {
      ...
      backend.create(Users.fromUser(User.copyOf(user).updateMillis(updateTime).build()));
  ...
  public Response modifyUser(@PathParam("name") String name, User user,
    ...
    try {
      ...
      backend.update(Users.fromUser(User.copyOf(user).updateMillis(updateTime).build()));
  ...
  public static com.dremio.iceberg.auth.User fromUser(User user) {
    return new com.dremio.iceberg.auth.User(user);
  }

  public static User toUser(com.dremio.iceberg.auth.User user) {
    return user.unwrap();
  }
}
