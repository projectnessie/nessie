...
public class ListTables {
  ...
  public Table[] getTables(@Parameter(description = "namespace in which to search", required = false)
                            @QueryParam("namespace") String namespace) {
    return backend.tableBackend().getAll(namespace, false).toArray(new Table[]{});
  }
  ...
  public Response getTable(@PathParam("name") String name) {
    ...
    EntityTag eTag = new EntityTag(Long.toString(table.getVersion().orElse(0)));
  ...
  public Response getTableByName(@PathParam("name") String name, @QueryParam("namespace") String namespace) {
    ...
    if (table.isPresent() && !table.get().isDeleted()) {
      EntityTag eTag = new EntityTag(Long.toString(table.get().getVersion().orElse(0)));
      return Response.ok(table.get()).tag(eTag).build();
    }
    table = backend.tableBackend().getAll(namespace, false).stream()
      .filter(t -> t.getId().equals(name)).findFirst();
    if (table.isPresent() && !table.get().isDeleted()) {
      EntityTag eTag = new EntityTag(Long.toString(table.get().getVersion().orElse(0)));
      return Response.ok(table.get()).tag(eTag).build();
    }
  ...
  public Response setTable(Table table) {
    try {
      Optional<Table> tableExisting = backend.tableBackend().getAll(table.getNamespace(), false).stream()
        ...
        .filter(t -> Objects.equals(t.getNamespace(), table.getNamespace())).findFirst();
      ...
      backend.tableBackend().create(id, newTable);
  ...
  public Response deleteTable(
    ...
    try {
      ...
      if (purge) {
        ...
        long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
        table = backend.tableBackend().get(name).withUpdateTime(updateTime).withDeleted(true);
  ...
  public Response updateTable(@PathParam("name") String name, @Context Request request, Table table) {
    EntityTag eTag = new EntityTag(Long.toString(backend.tableBackend().get(name).getVersion().orElse(0)));
    Response.ResponseBuilder evaluationResultBuilder = request.evaluatePreconditions(eTag);
    if (evaluationResultBuilder == null) {
      try {
        long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
        Table newTable = table.incrementVersion().withUpdateTime(updateTime);
        backend.tableBackend().update(name, newTable);
        return Response.status(200).build();
      } catch (Throwable t) {
        return Response.status(404, "something went wrong").build();
      }
