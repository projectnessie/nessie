...
public class ListTables {
  ...
  public Response getTableByName(@PathParam("name") String name,
                                 @QueryParam("namespace") String namespace) {
    Optional<VersionedWrapper<Table>> table =
      backend.tableBackend().getAll(namespace, false).stream()
             .filter(t -> t.getObj().getTableName().equals(name)).findFirst();
  ...
  public Response setTable(Table table) {
    try {
      Optional<VersionedWrapper<Table>> tableExisting = backend.tableBackend()
                                                               .getAll(table.getNamespace(), false).stream()
                                                               .filter(t -> t.getObj().getTableName().equals(table.getTableName()))
                                                               .filter(t -> Objects.equals(t.getObj().getNamespace(), table.getNamespace())).findFirst();
      if (tableExisting.isPresent()) {
        return Response.status(409).build();
      }
      String id = UUID.randomUUID().toString();
      long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
      Table newTable = Table.copyTable(table)
        .id(id)
  ...
  public Response updateTable(@PathParam("name") String name, @Context Request request,
    ...
    EntityTag eTag = new EntityTag(Long.toString(currentTable.getVersion().orElse(0)));
    Response.ResponseBuilder evaluationResultBuilder = request.evaluatePreconditions(eTag);
    if (evaluationResultBuilder == null) {
      try {
        long updateTime = ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli();
        Table newTable = Table.copyTable(table).updateTime(updateTime).build();
