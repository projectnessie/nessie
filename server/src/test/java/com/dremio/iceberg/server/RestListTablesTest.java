...
public class RestListTablesTest extends JerseyTest {
  ...
  public void testNamespace() {
    Response response = target("tables").request(MediaType.APPLICATION_JSON_TYPE)
                                        ...
                                        .post(Entity.entity(ImmutableTable.builder()
                                                                          .tableName("test1")
                                                                          .namespace("a.b.c")
                                                                          .baseLocation("loc")
  ...
  public void duplicateTest() {
    ...
    // update entity
    Table newTable = ImmutableTable.copyOf(table.getObj()).withBaseLocation("loc2");
    response = target(tableName).request(MediaType.APPLICATION_JSON)
                                .header(HttpHeaders.AUTHORIZATION, authHeader)
  ...
  public void optimisticLocking() {
    ...
    tableU1 = tableU1.update(ImmutableTable.copyOf(tableU1.getObj())
                                           .withMetadataLocation("foobar"));
    response = target(tableName).request(MediaType.APPLICATION_JSON)
    ...
    tableU2 = tableU2.update(ImmutableTable.copyOf(tableU2.getObj())
                                           .withMetadataLocation("foobaz"));
    response = target(tableName).request(MediaType.APPLICATION_JSON)
                                .header(HttpHeaders.AUTHORIZATION, authHeader)
