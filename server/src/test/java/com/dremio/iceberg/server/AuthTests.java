...
public class AuthTests {
  ...
  public void testAdmin() {
    ...
    List<Table> tables = client.getTableClient().getAll();
    Assert.assertTrue(tables.isEmpty());
    tryEndpointPass(() -> client.getTableClient().createObject(new Table("x", "x")));
    tryEndpointPass(() -> client.getTableClient().updateObject(new Table("x", "x")));
    tryEndpointPass(() -> client.getTableClient().deleteObject("x", false));
    Table table = client.getTableClient().getObject("x");
  ...
  public void testUser() {
    ...
    List<Table> tables = client.getTableClient().getAll();
    Assert.assertTrue(tables.isEmpty());
    tryEndpointFail(() -> client.getTableClient().createObject(new Table("x", "x")));
    tryEndpointFail(() -> client.getTableClient().updateObject(new Table("x", "x")));
