  ...
  public void testBasicTag() {
    ...
    Assert.assertNotEquals(getTag(catalog, foobar).getMetadataLocation(foobar),
      getTag(newCatalog, foobar)
        .getMetadataLocation(foobar)); // metadata location changed no longer matches
    Assert.assertEquals(initialMetadataLocation,
      getTag(newCatalog, foobar)
        .getMetadataLocation(foobar)); // points to the previous metadata location
    ...
    Assert.assertEquals(getTag(catalog, foobaz).getMetadataLocation(foobaz),
      getTag(newCatalog, foobaz)
        .getMetadataLocation(foobaz)); // metadata location changed no longer matches
    Assert.assertNotEquals(initialMetadataLocation,
      getTag(catalog, foobaz)
        .getMetadataLocation(foobaz)); // points to the previous metadata location
