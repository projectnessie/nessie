...
public class AlleyTableTest {
  ...
  private com.dremio.iceberg.model.Table getTable(TableIdentifier tableIdentifier) {
    return client.getTableClient().getObjectByName(tableIdentifier.name(), tableIdentifier.namespace().toString());
  }
  ...
  public void testFailure() {
    Table icebergTable = catalog.loadTable(TABLE_IDENTIFIER);
    com.dremio.iceberg.model.Table table = client.getTableClient().getObjectByName(TABLE_NAME, DB_NAME);
    client.getTableClient().updateObject(table.withMetadataLocation("dummytable"));
