...
public class AlleyTableTest {
  ...
  private com.dremio.iceberg.model.Table getTable(TableIdentifier tableIdentifier) {
    return client.getTableClient()
                 .getObjectByName(tableIdentifier.name(), tableIdentifier.namespace().toString());
  }
  ...
  public void testDropWithoutPurgeLeavesTableData() throws IOException {
    ...
    List<GenericData.Record> records = Lists.newArrayList(recordBuilder.set("id", 1L).build(),
    ...
    Assert.assertTrue("Drop should return true and drop the table",
                      catalog.dropTable(TABLE_IDENTIFIER,
                                        false /* do not delete underlying files */));
    Assert.assertFalse("Table should not exist", catalog.tableExists(TABLE_IDENTIFIER));
  ...
  public void testFailure() {
    Table icebergTable = catalog.loadTable(TABLE_IDENTIFIER);
    com.dremio.iceberg.model.Table table =
        client.getTableClient().getObjectByName(TABLE_NAME, DB_NAME);
    client.getTableClient()
          .updateObject(com.dremio.iceberg.model.Table.copyTable(table)
                                                      .metadataLocation("dummytable")
                                                      .build());
