...
public class AlleyCatalog extends BaseMetastoreCatalog implements TagCatalog, Closeable {
  ...
  private Table tableFromTableIdentifier(TableIdentifier tableIdentifier) {
    if (tableIdentifier.hasNamespace()) {
      return Table.table(tableIdentifier.name(), tableIdentifier.namespace().toString(), warehouseLocation);
    } else {
      return Table.table(tableIdentifier.name(), warehouseLocation);
    }
  }
  ...
  public List<TableIdentifier> listTables(Namespace namespace) {
    Table[] tables = client.getTableClient().getAll(namespace == null ? null : namespace.toString());
  ...
  public Tag createTag(String tagName, String parentName) {
    com.dremio.iceberg.model.Tag newTag = com.dremio.iceberg.model.Tag.tag(tagName, parentName);
