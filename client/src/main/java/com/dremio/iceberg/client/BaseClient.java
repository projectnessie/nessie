...
abstract class BaseClient<T extends Base> {
  ...
  public void updateObject(T table) {
    try (AutoCloseableClient client = new AutoCloseableClient()) {
      ...
      if (table.getVersion().isPresent()) {
        request.header(HttpHeaders.IF_MATCH, new EntityTag(Long.toString(table.getVersion().getAsLong())));
      }
...
//    table.setUpdateTime(ZonedDateTime.now(ZoneId.of("UTC")).toInstant().toEpochMilli());
    try (AutoCloseableClient client = new AutoCloseableClient()) {
      ...
      assert newTable.getVersion().orElse(0L) == Long.parseLong(((String) response.getHeaders()
        .getFirst(HttpHeaders.ETAG)).replaceAll("\"", ""));
      return newTable;
    }
  }
  ...
  static class AutoCloseableClient implements AutoCloseable {
    ...
    public Invocation.Builder get(String endpoint, String path, String mediaType, String authHeader,
      ...
      Invocation.Builder builder = webTarget.path(path).request(mediaType).accept(MediaType.APPLICATION_JSON_TYPE);
      if (authHeader != null) {
