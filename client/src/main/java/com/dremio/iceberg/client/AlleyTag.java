...
public class AlleyTag implements Tag {
  ...
  public void commit(Tag old) {
    Map<String, TableVersion> snapshots = old.tables()
                                              .stream()
                                              .map(t -> setTableVersion(t,
                                                                        old.getMetadataLocation(t)))
                                              .collect(Collectors.toMap(TableVersion::getUuid,
                                                                        Function.identity()));
    com.dremio.iceberg.model.Tag newTag = com.dremio.iceberg.model.Tag.copyOf(tag)
  ...
  private Table getTable(TableIdentifier tableIdentifier) {
    return tableCache.computeIfAbsent(
      tableIdentifier, t -> client.getTableClient()
  ...
  private TableVersion getTableVersion(Table table) {
    ...
    boolean exists = table.tableVersions().stream().map(table::getTableVersion)
