...
public class NessieCatalog extends BaseMetastoreCatalog implements BranchCatalog, Closeable {
  ...
  public NessieCatalog(Configuration config) {
    ...
    branch = new AtomicReference<>(client.getBranch(config.get("nessie.view-branch",
                                                               client.getConfig()
                                                                     .getDefaultTag())));
  }
  ...
  public List<TableIdentifier> listTables(Namespace namespace) {
    Table[] tables = client.getAllTables(branch.get().getName(),
  ...
  public boolean dropTable(TableIdentifier identifier, boolean purge) {
    ...
    client.deleteTable(branch.get(), existingTable.getId(), purge);
  ...
  @Override
  public com.dremio.nessie.client.branch.Branch createBranch(String branchName, String parentName) {
    com.dremio.nessie.model.Branch newBranch = ImmutableBranch.builder()
                                                              .name(branchName)
                                                              .id(parentName)
                                                              .build();
    return new NessieBranch(client.createBranch(newBranch), client);
  }
  ...
  @Override
  public void promoteBranch(String from, String to, boolean force) {
    ...
    if (existingToTable == null) {
      throw new NoSuchTableException("branch %s already exists", to);
    }
    try {
      client.updateBranch(existingToTable, existingFromTable.getName(), force);
    } catch (Throwable t) {
  ...
  @Override
  public com.dremio.nessie.client.branch.Branch loadBranch(String branchName) {
    return new NessieBranch(client.getBranch(branchName), client);
  }
