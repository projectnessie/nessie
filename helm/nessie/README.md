...
| gc.args | list | `[]` | Arguments for the GC container, replacing the image's default arguments. Use this to invoke the desired GC subcommand (for example `gc`) together with its options. At least one of `gc.command`, `gc.args`, or `gc.extraArgs` is required when `gc.enabled` is true. |
