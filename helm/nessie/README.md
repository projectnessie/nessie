...
| tracing.sampler | bool | `true` | The sampler to use for tracing. Valid values are: false (never sample); true (always sample); or the string "ratio" – in which case the actual ratio to use must be specified below. Defaults to true. |
