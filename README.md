...
[![Build Status](https://travis-ci.org/dremio/iceberg-alley.svg?branch=master)](https://travis-ci.org/dremio/iceberg-alley)

Iceberg Alley is a metadata store for [Iceberg](https://iceberg.incubator.apache.org/) that can function the same as the
...
Iceberg Alley is a lightweight and low resource service that can run standalone, embedded in another application or in a
...
Iceberg Alley also adds some features not found in other metadata stores including:
...
Iceberg Alley is composed of several components:
* **Server** - the Iceberg Alley metadata store. A standard REST api. This can be deployed as part
of another java based application, as a standalone service or as a serverless application
* **Client** - the Iceberg Alley client for use in Iceberg
...
* **Delta Lake** - experimental Delta Lake `LogStore` using Iceberg Alley for atomic metadata updates

## Installation

Clone this repository and run maven:
```bash
git clone https://github.com/dremio/iceberg-alley
cd iceberg-alley
mvn clean install
```
...
2. execute `mvn exec:exec -pl :iceberg-alley-distribution`
...
```xml
<dependency>
  <groupId>com.dremio.iceberg</groupId>
  <artifactId>iceberg-alley-server</artifactId>
...
You can also deploy to AWS lambda function by deploying the `iceberg-alley-serverless-1.0-SNAPSHOT.jar` as a lambda 
function
...
database for testing. To implement a new backend simply implement `com.dremio.iceberg.backend.Backend` and ensure the 
...
The Iceberg Alley service expects that if a create or update operation completes successfully then it should be atomic 
...
To create a new backend implement `com.dremio.iceberg.auth.UserService` and add it to the classpath.
