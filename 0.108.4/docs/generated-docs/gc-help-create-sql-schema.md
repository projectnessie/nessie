...
JDBC schema creation.
  -h, --help               Show this help message and exit.
      ...
      --jdbc-pool-max-size=<poolMaxSize>
                           Maximum JDBC connection pool size, defaults to 5. The sweep/expire phase
                             holds one streaming connection plus one per --expiry-parallelism
                             worker, so set this to at least --expiry-parallelism + 1 to avoid
