...
public class Tag implements Base {
  ...
  public Tag(String name, long createMillis, Long expireMillis, String uuid, boolean deleted, Map<String, TableVersion> tableSnapshots, String baseTag, Long version, long updateTime) {
    this.name = name;
  ...
  public static Tag fromModelTag(com.dremio.iceberg.model.Tag oldTag) {
    Tag tag = new Tag(
      ...
      oldTag.getExpireMillis().isPresent() ? oldTag.getExpireMillis().getAsLong() : null,
      oldTag.getId(),
      oldTag.isDeleted(),
      oldTag.tableSnapshotKeys().stream().collect(Collectors.toMap(Function.identity(), oldTag::getTableSnapshot)),
      oldTag.getBaseTag(),
      oldTag.getVersion().isPresent() ? oldTag.getVersion().getAsLong() : null,
