...
public class Table implements Base {
  ...
  public static Table fromModelTable(com.dremio.iceberg.model.Table oldTable) {
    Table table = new Table(
      ...
      oldTable.getSchema(),
      oldTable.getVersion().isPresent() ? oldTable.getVersion().getAsLong() : null,
      oldTable.getUpdateTime(),
      oldTable.tableVersions().stream()
        .collect(Collectors.toMap(Function.identity(), oldTable::getTableVersion)));
    return table;
  }
   ...
   */
  public static class VersionTypeConverter implements AttributeConverter<Map<String, TableVersion>> {
    ...
    public AttributeValue transformFrom(Map<String, TableVersion> stringTableVersionMap) {
      ...
      for (Map.Entry<String, TableVersion> e : stringTableVersionMap.entrySet()) {
        try {
          attributeMap.put(e.getKey(), AttributeValue.builder().s(MAPPER.writeValueAsString(e.getValue())).build());
