...
class ITCheckContentPersist extends BaseContentPersistTest<CheckContentEntry> {
  ...
  @Test
  public void testPersistError(QuarkusMainLauncher launcher, Persist persist) throws Exception {
    ...
    assertThat(entries)
        ...
        .containsExactly(k1, "ERROR", null, "Failure parsing data", true);
    assertThat(result.exitCode()).isEqualTo(EXIT_CODE_CONTENT_ERROR);
  }

  @ParameterizedTest
  @ValueSource(ints = {1, 2, 3, 4, 5, 10})
  public void testWorkerError(int batchSize, QuarkusMainLauncher launcher, Persist persist)
      throws Exception {

    ByteString broken = ByteString.copyFrom(new byte[] {1, 2, 3});
    commit(table1, broken, persist);
    commit(table2, broken, persist);
    commit(table3, broken, persist);
    commit(table4, broken, persist);
  ...
  @Test
  public void testValidData(QuarkusMainLauncher launcher, Persist persist) throws Exception {

    commit(table1, persist);
    commit(table2, persist);
  ...
  @Test
  public void testValidDataNoContent(QuarkusMainLauncher launcher, Persist persist)
      throws Exception {
    commit(table1, persist);
    commit(table2, persist);
  ...
  @Test
  public void testValidDataStdOut(QuarkusMainLauncher launcher, Persist persist) throws Exception {
    commit(table1, persist);
    commit(table2, persist);
  ...
  @Test
  public void testErrorOnly(QuarkusMainLauncher launcher, Persist persist) throws Exception {
    commit(table1, persist);
  ...
  @Test
  public void testHashWithBrokenCommit(QuarkusMainLauncher launcher, Persist persist)
      throws Exception {
    commit(table1, persist);

    Reference good = persist.fetchReference("refs/heads/main");
    ...
    launch(launcher, "check-content", "--hash", objIdToHash(good.pointer()).asString());
    assertThat(entries.stream().sorted(Comparator.comparing(CheckContentEntry::getKey)))
