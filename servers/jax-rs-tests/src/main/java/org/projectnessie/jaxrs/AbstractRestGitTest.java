...
public class AbstractRestGitTest {
  ...
  public void testBasic() {
    ...
    IcebergSnapshot table = IcebergSnapshot.of("/the/directory/over/there", -1L);
    ...
    rest()
        .queryParam("sourceRef", b3.getName())
  ...
  private Branch commit(Branch branch, String contentsKey, String metadataUrl, String author) {
    Operations contents =
        ImmutableOperations.builder()
            .addOperations(
                Put.of(ContentsKey.of(contentsKey), IcebergSnapshot.of(metadataUrl, -1L)))
  ...
  private Branch makeBranch(String name) {
    rest().body(Branch.of(name, null)).post("trees/tree").then().statusCode(200);
    return ImmutableBranch.builder().name(name).build();
  }
