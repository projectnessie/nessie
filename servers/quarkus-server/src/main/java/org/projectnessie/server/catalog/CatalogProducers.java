...
public class CatalogProducers {
  ...
  public EntitySnapshotTaskBehavior entitySnapshotTaskBehavior(
      CatalogServiceConfig config, ObjectIOExceptionMapper mapper) {
    return new EntitySnapshotTaskBehavior(mapper, config.importTaskTimeout());
  }

  @Produces
  @Singleton
  public ObjectIOExceptionMapper objectIOExceptionMapper(CatalogServiceConfig config) {
    return ObjectIOExceptionMapper.builder()
        .clock(systemUTC())
        .retryAfterThrottled(config.retryAfterThrottled())
        .retryAfterNetworkError(config.retryAfterNetworkError())
        .reattemptAfterFetchError(config.reattemptAfterFetchError())
