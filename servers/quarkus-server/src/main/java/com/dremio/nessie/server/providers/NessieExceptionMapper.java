...
public class NessieExceptionMapper implements ExceptionMapper<Exception> {
  ...
  public Response toResponse(Exception exception) {
    ...
    if (exception instanceof WebApplicationException) {
      Response.Status status =
          Status.fromStatusCode(((WebApplicationException) exception).getResponse().getStatus());
      ...
      return exception(
          ((BaseNessieClientServerException) exception).getStatus(),
  ...
  private Response exception(Status status, String message, Exception e) {
    String stack =
        serverConfig.shouldSendstackTraceToAPIClient() ? Throwables.getStackTraceAsString(e) : null;
