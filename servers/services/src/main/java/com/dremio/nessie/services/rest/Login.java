...
public class Login {
  ...
  public Response login(@FormParam("username") String login,
    ...
    if (loginService instanceof NoopLoginService) {
    ...
    try {
      ...
      String token = loginService.authorize(login, password);
      return Response.ok(ImmutableAuthResponse.builder().token(token).expiryDate("").build())
