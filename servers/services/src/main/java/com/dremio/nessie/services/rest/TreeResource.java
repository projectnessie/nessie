...
public class TreeResource extends BaseResource implements TreeApi {
  ...
  @Override
  public void assignReference(@NotNull String ref, ReferenceUpdate reference)
  ...
  @Override
  public void mergeRefIntoBranch(@NotNull String branchName, @NotNull Merge merge)
      throws NessieNotFoundException, NessieConflictException {
    try {
  ...
  private static Reference makeRef(WithHash<? extends Ref> refWithHash) {
    ...
    if (ref instanceof TagName) {
      ...
      return ImmutableHash.builder().name(hash).hash(hash).build();
    } else {
