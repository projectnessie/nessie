...
@RequestScoped
@Path("objects")
public class TableBranchOperations2 implements RefOperationsApi, ContentsApi {

  private static final Logger logger = LoggerFactory.getLogger(TableBranchOperations2.class);

  private final VersionStoreAdapter backend;

  @Inject
  public TableBranchOperations2(VersionStoreAdapter backend) {
  ...
  public Reference ref2(
      @Parameter(description = "name of ref to fetch", required = true) @PathParam("ref") String refName) {
    throw new UnsupportedOperationException();
  }
  ...
  public Response refTable(@Parameter(description = "name of ref to search on", required = true)
    ...
    Contents table = backend.getTableOnReference(ref, tableName);
    if (table == null) {
  ...
  public Response createTable(@Parameter(description = "ref on which table will be created", required = true)
                              ...
                              @RequestBody(description = "table object to be created",
                                content = @Content(schema = @Schema(implementation = Contents.class)))
                                Contents table)
      throws ReferenceNotFoundException, ReferenceConflictException, ReferenceAlreadyExistsException {
    Contents existing = backend.getTableOnReference(ref, tableName);
    if (existing != null) {
  ...
  public Response update(@Parameter(description = "ref on which to add merges", required = true)
                         ...
                         @Context SecurityContext securityContext,
                         @Context HttpHeaders headers,
                         Contents update) throws ReferenceConflictException, ReferenceNotFoundException {
  ...
  public Response updateMulti(@Parameter(description = "ref on which to add merges", required = true)
    ...
    String hash = version(headers).orElseThrow(() -> new ReferenceConflictException("ETag header not supplied"));
    Principal principal = securityContext.getUserPrincipal();
    List<com.dremio.nessie.versioned.Operation<Contents>> ops = Arrays.stream(update).map(t -> {
      Key key = VersionStoreAdapter.keyFromUrlString(t.getId());
      return t.isDeleted() ? Delete.<Contents>of(key) : Put.of(key, t);
