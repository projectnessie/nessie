...
export class DefaultApi extends runtime.BaseAPI {
    ...
    /**
     * Retrieve the commit log for a ref, potentially truncated by the backend.  Retrieves up to \'maxRecords\' commit-log-entries starting at the HEAD of the given named reference (tag or branch) or the given hash. The backend may respect the given \'max\' records hint, but return less or more entries. Backends may also cap the returned entries at a hard-coded limit, the default REST server implementation has such a hard-coded limit.  To implement paging, check \'hasMore\' in the response and, if \'true\', pass the value returned as \'token\' in the next invocation as the \'pageToken\' parameter.  The content and meaning of the returned \'token\' is \"private\" to the implementation,treat is as an opaque value.  It is wrong to assume that invoking this method with a very high \'maxRecords\' value will return all commit log entries.
     ...
     */
    async getReferenceByNameRaw(requestParameters: GetReferenceByNameRequest): Promise<runtime.ApiResponse<void>> {
        if (requestParameters.ref === null || requestParameters.ref === undefined) {
     ...
     * Fetch details of a reference
     */
    async getReferenceByName(requestParameters: GetReferenceByNameRequest): Promise<void> {
        await this.getReferenceByNameRaw(requestParameters);
    }
