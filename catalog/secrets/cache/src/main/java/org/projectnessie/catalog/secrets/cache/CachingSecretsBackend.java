...
public class CachingSecretsBackend {
  ...
  private static final Secret CACHE_NEGATIVE_SENTINEL = new Secret() {};
  ...
  private int weigher(CacheKeyValue key, Secret value) {
    int size = key.heapSize();
    if (isPresentValue(value)) {
      // We assume 1kB for every secret (which is probably way more than needed)
      size += 1024 + MAP_OBJ_OVERHEAD;
    }
    size += CAFFEINE_OBJ_OVERHEAD;
    return size;
  }

  private static boolean isPresentValue(Secret value) {
    // Yes, same instance for
    return value != null && value != CACHE_NEGATIVE_SENTINEL;
  }

  Map<String, Secret> resolveSecrets(
      String repositoryId, Map<String, SecretType> toResolve, SecretsSupplier backend) {
    long ttl = ttlNanos;
    long expires = ttl != 0L ? clock.getAsLong() + ttl : 0L;

    Map<String, CacheKeyValue> keysToResolve = new HashMap<>();
    for (String name : toResolve.keySet()) {
      keysToResolve.put(name, new CacheKeyValue(repositoryId, name, expires));
    }

    Map<String, Secret> result = new HashMap<>();

    Map<CacheKeyValue, Secret> present = cache.getAllPresent(keysToResolve.values());
    present.forEach(
        (k, v) -> {
          String name = k.name;
          keysToResolve.remove(name);
          if (isPresentValue(v)) {
            result.put(name, v);
          }
        });

    Map<String, SecretType> resolve =
        keysToResolve.keySet().stream()
            .collect(Collectors.toMap(Function.identity(), toResolve::get));

    if (!keysToResolve.isEmpty()) {
      Map<String, Secret> resolved = backend.resolveSecrets(resolve);

      Map<CacheKeyValue, Secret> put = new HashMap<>();
      for (CacheKeyValue key : keysToResolve.values()) {
        Secret value = resolved.getOrDefault(key.name, CACHE_NEGATIVE_SENTINEL);
        if (isPresentValue(value)) {
  ...
  private static String safeStringValue(Map.Entry<String, ?> e) {
