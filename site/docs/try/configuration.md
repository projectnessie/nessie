...
| `nessie.version.store.advanced.max-key-list-entity-size`        | `1_000_000`         | `int`    | Sets the maximum size of a database object/row. This parameter is respected for `CommitLogEntry` and `KeyList`. This value must not be "on the edge" - means: it must leave enough room for a somewhat large-ish list |
