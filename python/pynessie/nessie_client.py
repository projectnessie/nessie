...
class NessieClient(object):
    ...
    def list_keys(
        ...
        return EntriesSchema().load(
            list_tables(self._base_url, ref, hash_on_ref, max_result_hint, page_token, query_expression, self._ssl_verify)
        )
    ...
    def get_values(self: "NessieClient", ref: str, *tables: str, hash_on_ref: Optional[str] = None) -> Generator[Contents, Any, None]:
        ...
        :return: Nessie Table
        """
        return (ContentsSchema().load(get_table(self._base_url, ref, _format_key(i), hash_on_ref, self._ssl_verify)) for i in tables)
    ...
    def assign_branch(
        ...
        assign_branch(self._base_url, branch, ref_json, old_hash, self._ssl_verify)
    ...
    def assign_tag(self: "NessieClient", tag: str, to_ref: str, to_ref_hash: Optional[str] = None, old_hash: Optional[str] = None) -> None:
        ...
        assign_tag(self._base_url, tag, ref_json, old_hash, self._ssl_verify)
    ...
    def get_log(
        ...
        def fetch_logs(token: Optional[str] = page_token) -> LogResponse:
            ...
            fetched_logs = list_logs(
                base_url=self._base_url, hash_on_ref=hash_on_ref, ref=start_ref, ssl_verify=self._ssl_verify, **filtering_args
            )
