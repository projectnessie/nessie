...
def handle_branch_tag(
    ...
    if list_references or (not list_references and not delete_reference and not branch and not new_branch):
        ...
        if not hash_:
            hash_ = nessie.get_reference(branch).hash_
        getattr(nessie, "delete_{}".format("branch" if is_branch else "tag"))(branch, hash_)
        ...
        try:
            getattr(nessie, "assign_{}".format("branch" if is_branch else "tag"))(branch, new_branch, hash_, old_hash)
        except NessieNotFoundException:
            if not hash_:
                hash_ = nessie.get_reference(new_branch).hash_
            getattr(nessie, "create_{}".format("branch" if is_branch else "tag"))(branch, new_branch, hash_)
    else:
        getattr(nessie, "assign_{}".format("branch" if is_branch else "tag"))(branch, new_branch, hash_, old_hash)
    return ""
