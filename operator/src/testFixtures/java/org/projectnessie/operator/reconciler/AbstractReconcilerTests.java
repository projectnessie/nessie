...
public abstract class AbstractReconcilerTests<T extends HasMetadata> {
  ...
  void createAndDelete() {
    ...
    await()
        ...
        .atMost(timeout())
        .ignoreExceptions()
        .untilAsserted(this::assertResourcesCreated);
    waitForPrimaryReady();
    setUpFunctionalTest();
    LOGGER.info(
    ...
    await()
        ...
        .atMost(timeout())
        .ignoreExceptions()
        .untilAsserted(this::functionalTest);
    LOGGER.info(
    ...
    await()
        ...
        .atMost(timeout())
        .ignoreExceptions()
        .untilAsserted(this::assertResourcesDeleted);
  }
  ...
  protected <R extends HasMetadata, L extends KubernetesResourceList<R>> L list(
      MixedOperation<R, L, ? extends Resource<R>> resources) {
    return resources.inNamespace(namespace.getMetadata().getName()).list();
  }
  ...
  protected void checkConfigMap(ConfigMap expected, ConfigMap actual) {
    assertThat(actual).isNotNull();
    checkMeta(expected, actual);
