...
import io.fabric8.kubernetes.api.model.networking.v1beta1.IngressLoadBalancerIngress;
...
import io.fabric8.kubernetes.api.model.networking.v1beta1.IngressStatus;
...
public class IngressV1Beta1Dependent extends AbstractIngressDependent<Ingress> {

  public IngressV1Beta1Dependent() {
    super(Ingress.class);
  }
    ...
    for (Rule rule : rules) {
      ...
      for (String path : rule.paths()) {
        ruleBuilder
            ...
            .withNewBackend()
            .withServiceName(nessie.getMetadata().getName())
            .withNewServicePort()
            .withValue(nessie.getSpec().service().port())
            .endServicePort()
            .endBackend()
            .endPath()
            .endHttp();
  ...
  private void configureTls(Ingress ingress, Nessie nessie) {
    for (Tls tls : nessie.getSpec().ingress().tls()) {
      ...
      tlsBuilder.withSecretName(tls.secretRef().getName());
