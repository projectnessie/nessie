...
public record VersionStoreCacheOptions(
    ...
    @JsonPropertyDescription(
            """
            The percentage of the available heap that the cache should use. \
            The default is 700m (70%). Must be > 0 and < 1000m.""")
        @Default("700m")
        Quantity heapPercentage,
    @JsonPropertyDescription(
            """
  ...
  public VersionStoreCacheOptions {
    ...
    heapPercentage = heapPercentage != null ? heapPercentage : DEFAULT_HEAP_PERCENTAGE;
  ...
  public void validate() {
    if (enabled) {
      if (heapPercentage.getNumericalAmount().compareTo(BigDecimal.ZERO) <= 0
          || heapPercentage.getNumericalAmount().compareTo(BigDecimal.ONE) >= 0) {
        throw new InvalidSpecException(
            InvalidVersionStoreConfig,
            "Invalid cache configuration: spec.versionStore.cache.heapPercentage must be > 0 and < 1");
      }
