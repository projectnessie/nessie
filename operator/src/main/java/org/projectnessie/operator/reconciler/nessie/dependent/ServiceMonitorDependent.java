...
public class ServiceMonitorDependent
  ...
  public ServiceMonitor desired(Nessie nessie, Context<Nessie> context) {
    ...
    return new ServiceMonitorBuilder()
        .withMetadata(
            helper
                .metaBuilder(nessie)
                .addToLabels(nessie.getSpec().serviceMonitor().labels())
                .build())
        .withNewSpec()
        .addNewEndpoint()
        .withPort("nessie-server")
        .withScheme("http")
        .withInterval(nessie.getSpec().serviceMonitor().interval())
        .withPath("/q/metrics")
        .endEndpoint()
  ...
  public static class ActivationCondition implements Condition<ServiceMonitor, Nessie> {
    ...
    public boolean isMet(
      ...
      boolean conditionMet = nessie.getSpec().serviceMonitor().enabled();
      KubernetesHelper helper = KubernetesHelper.retrieveFromContext(context);
