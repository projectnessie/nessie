...
@QuarkusTestResource(
    value = PostgresContainerLifecycleManager.class,
    parallel = true,
    restrictToAnnotatedClass = true)
class ITNessieReconcilerJdbc extends AbstractNessieReconcilerIntegrationTests {

  private static final String PREFIX = "/org/projectnessie/operator/it/nessie/jdbc/";
  ...
  protected void assertResourcesCreated() {
    ...
    checkConfigMap(
        load(client.configMaps(), PREFIX + "config-map.yaml")
            .edit()
            .addToData("QUARKUS_DATASOURCE_JDBC_URL", jdbcUrl)
            .build(),
  ...
  protected void assertResourcesDeleted() {
    ...
    assertThat(get(client.autoscaling().v2().horizontalPodAutoscalers(), "nessie-test")).isNull();
