...
import static org.projectnessie.operator.events.EventReason.CreatingHorizontalPodAutoscaler;
...
@WithKubernetesTestServer(setup = TestNessieReconcilerMongo.Setup.class)
class TestNessieReconcilerMongo extends AbstractReconcilerUnitTests<Nessie> {

  private static final String PREFIX = "/org/projectnessie/operator/tests/nessie/mongo/";
  ...
  protected void assertResourcesCreated() {
    ...
    checkAutoscaler(
        load(client.autoscaling().v2beta2().horizontalPodAutoscalers(), PREFIX + "autoscaler.yaml"),
        get(client.autoscaling().v2beta2().horizontalPodAutoscalers(), "nessie-test"));
    checkEvents(
        ...
        CreatingHorizontalPodAutoscaler,
        ReconcileSuccess);
  ...
  public static class Setup extends AbstractReconcilerUnitTests.Setup {
    ...
    public void accept(KubernetesServer server) {
      ...
      reportApiSupported(server, "autoscaling", "v2beta2");
