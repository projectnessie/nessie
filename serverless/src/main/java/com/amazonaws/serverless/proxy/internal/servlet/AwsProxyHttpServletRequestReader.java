...
/**
 * Simple implementation of the <code>RequestReader</code> interface that receives an
 * <code>AwsProxyRequest</code> object and uses it to initialize a <code>AwsProxyHttpServletRequest</code>
 * object.
 */
public class AwsProxyHttpServletRequestReader extends
                                              RequestReader<AwsProxyRequest, HttpServletRequest> {
  ...
  public HttpServletRequest readRequest(AwsProxyRequest request,
    ...
    if (request.getMultiValueHeaders() != null
        && request.getMultiValueHeaders().getFirst(HttpHeaders.CONTENT_TYPE) != null) {
      ...
      request.getMultiValueHeaders()
             .putSingle(HttpHeaders.CONTENT_TYPE, getContentTypeWithCharset(contentType, config));
    }
    AwsProxyHttpServletRequest servletRequest = new AwsProxyHttpServletRequest(request,
                                                                               lambdaContext,
                                                                               securityContext,
                                                                               config);
