...
/**
 * Implementation of the <code>FilterChain</code> interface. FilterChainHolder objects should be
 * accessed through the
 * <code>FilterChainManager</code>. Once a filter chain is loaded, use the <code>doFilter</code>
 * method to run the chain
...
public class FilterChainHolder implements FilterChain {
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.servlet.FilterChainHolder.class);
  ...
  public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse)
    ...
    if (currentFilter <= filters.size() - 1) {
      ...
      if (!holder.getRegistration()
                 .getDispatcherTypes()
                 .contains(servletRequest.getDispatcherType())) {
   ...
   *
   * @return The number of filters in the chain holder. If the filter chain is null then this will
   * return 0
   */
  int filterCount() {
    if (filters == null) {
   ...
   *
   * @param idx The index in the chain. Use the <code>filterCount</code> method to get the filter
   *            count
  ...
  /**
   * Resets the chain holder to the beginning of the filter chain. This method is used from the
   * constructor as well as when the {@link FilterChainManager} return a holder from the cache.
   */
