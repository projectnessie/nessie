...
/**
 * Abstract extension of the code <code>LambdaContainerHandler</code> object that adds protected
 * variables for the
 * <code>ServletContext</code> and <code>FilterChainManager</code>. This object should be extended
 * by the framework-specific
 * implementations that want to support the servlet 3.1 specs.
 * <p>
 * Because Lambda only allows one event per container at a time, this object also acts as the
...
public abstract class AwsLambdaServletContainerHandler<RequestType, ResponseType,
  ...
  // Variables - Private
  //-------------------------------------------------------------
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.servlet.AwsLambdaServletContainerHandler.class);
  ...
  /**
   * You can use the <code>onStartup</code> to intercept the ServletContext as the Spring
   * application is initialized and inject custom values. The StartupHandler is called after the
   * <code>onStartup</code> method of the <code>LambdaSpringApplicationinitializer</code>
   * implementation. For example, you can use this method to add custom filters to the servlet
   * context:
   *
   * <pre>
   * {@code
   ...
   * </pre>
   *
   * @param h A lambda expression that implements the <code>StartupHandler</code> functional
   *          interface
   */
   ...
   *
   * @return The initialized servlet context if the framework-specific implementation requires one,
   * otherwise null
   */
  ...
  public void initialize() throws ContainerInitializationException {
    ...
    ArrayList<AwsServletRegistration> registrations = new ArrayList<>((Collection<AwsServletRegistration>) getServletContext()
    ...
    for (AwsServletRegistration r : registrations) {
      ...
      try {
        ...
        throw new ContainerInitializationException("Could not initialize servlet " + r.getName(),
                                                   e);
      }
    }
  }
