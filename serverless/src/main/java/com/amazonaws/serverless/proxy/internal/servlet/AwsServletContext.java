...
/**
 * Basic implementation of the <code>ServletContext</code> object. Because this library is not a
 * complete container implementation the majority of methods in this object return a
 * NotImplementedException or null. Supported properties are <code>initParameters</code>,
 * <code>attributes</code>, and <code>filters</code>.
 */
public class AwsServletContext
  ...
  public static final String SERVER_INFO =
    LambdaContainerHandler.SERVER_INFO + "/" + SERVLET_API_MAJOR_VERSION + "."
    + SERVLET_API_MINOR_VERSION;
  //-------------------------------------------------------------
  // Variables - Private - Static
  //-------------------------------------------------------------
  private static com.amazonaws.serverless.proxy.internal.servlet.AwsServletContext instance;
  //-------------------------------------------------------------
  // Variables - Private
  //-------------------------------------------------------------
  private final Map<String, FilterHolder> filters;
  private final Map<String, AwsServletRegistration> servletRegistrations;
  private final Map<String, Object> attributes;
  private final Map<String, String> initParameters;
  private final AwsLambdaServletContainerHandler containerHandler;
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.servlet.AwsServletContext.class);
  ...
  public URL getResource(String s) throws MalformedURLException {
    return com.amazonaws.serverless.proxy.internal.servlet.AwsServletContext.class.getResource(s);
  }


  @Override
  public InputStream getResourceAsStream(String s) {
    return com.amazonaws.serverless.proxy.internal.servlet.AwsServletContext.class.getResourceAsStream(
      s);
  }
    ...
    for (AwsServletRegistration reg : servletRegistrations.values()) {
      for (String p : reg.getMappings()) {
        ...
        for (int i = 0; i < regParts.length; i++) {
          ...
          if (i == regParts.length - 1 && (regParts[i].equals(pathParts[i])
                                           || "*" .equals(regParts[i]))) {
            return reg.getServlet();
          }
        }
  ...
  public String getRealPath(String s) {
    ...
    if (fileUrl != null) {
      try {
        ...
        log.error("Error while looking for real path {}: {}",
                  SecurityUtils.encode(s),
                  SecurityUtils.encode(e.getMessage()));
      }
    }
    return absPath;
  }
  ...
  public ServletRegistration.Dynamic addServlet(String s, String s1) {
    try {
      Class<? extends Servlet> servletClass = (Class<? extends Servlet>) this.getClassLoader()
                                                                             .loadClass(s1);
  ...
  public FilterRegistration.Dynamic addFilter(String name, Filter filter) {
    ...
    if (filters.containsKey(name)) {
      ...
      log.debug("Adding filter '{}' from {}",
                SecurityUtils.encode(name),
                SecurityUtils.encode(filter.toString()));
    }
  ...
  public FilterRegistration.Dynamic addFilter(String name, Class<? extends Filter> filterClass) {
    try {
      log.debug("Adding filter '{}' from {}",
                SecurityUtils.encode(name),
                SecurityUtils.encode(filterClass.getName()));
