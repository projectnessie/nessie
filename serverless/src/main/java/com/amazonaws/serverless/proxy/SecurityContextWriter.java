 ...
 * <code>SecurityContext</code> object from the incoming AWS Lambda event.
 *
...
public interface SecurityContextWriter<RequestType> {
  ...
  /**
   * Called by the container implementation to generate a <code>SecurityContext</code> given an
   * incoming event. The library includes a default implementation that reads from the AWS_PROXY
   * integration events.
   *
