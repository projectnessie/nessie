...
/**
 * Servlet filter class that calls Jersey's ApplicationHandler. Given a Jax RS Application object,
 * this class initializes a Jersey {@link ApplicationHandler} and calls its <code>handle</code>
 * method. Requests are transformed to <code>ContainerRequest</code> objects by the {@link
 * #servletRequestToContainerRequest(ServletRequest)} method. Jersey responses are written directly
 * to the ServletResponse by the {@link JerseyServletResponseWriter}.
 */
public class JerseyHandlerFilter implements Filter, Container {
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.jersey.JerseyHandlerFilter.class);
  private URI baseUri;
   ...
   * URI is built from the request's <code>getPathInfo()</code> method. The container request also
   * contains the API Gateway context, stage variables, and Lambda context properties. The original
   * servlet request is also embedded in a property of the container request to allow injection by
   * the {@link AwsProxyServletRequestSupplier}.
   *
  ...
  private ContainerRequest servletRequestToContainerRequest(ServletRequest request) {
    ...
    if (LambdaContainerHandler.getContainerConfig().getServiceBasePath() != null
        && LambdaContainerHandler.getContainerConfig().isStripBasePath()) {
      if (requestFullPath.startsWith(LambdaContainerHandler.getContainerConfig()
                                                           .getServiceBasePath())) {
        requestFullPath = requestFullPath.replaceFirst(LambdaContainerHandler.getContainerConfig()
                                                                             .getServiceBasePath(),
                                                       "");
    ...
    apiGatewayProperties.setProperty(API_GATEWAY_CONTEXT_PROPERTY,
                                     servletRequest.getAttribute(API_GATEWAY_CONTEXT_PROPERTY));
    apiGatewayProperties.setProperty(API_GATEWAY_STAGE_VARS_PROPERTY,
                                     servletRequest.getAttribute(API_GATEWAY_STAGE_VARS_PROPERTY));
    apiGatewayProperties.setProperty(LAMBDA_CONTEXT_PROPERTY,
                                     servletRequest.getAttribute(LAMBDA_CONTEXT_PROPERTY));
    ...
    while (headerNames.hasMoreElements()) {
      ...
      requestContext.getHeaders()
                    .addAll(headerKey, Collections.list(servletRequest.getHeaders(headerKey)));
    }
  ...
  /**
   * Shuts down and restarts the application handler in the current container. The
   * <code>ApplicationHandler</code> object is re-initialized with the <code>Application</code>
   * object initially set in the <code>LambdaContainer.getInstance()</code> call.
   */
  @Override
  public void reload() {
  ...
  /**
   * Restarts the application handler and configures a different <code>Application</code> object.
   * The new application resets the one currently configured in the container.
   *
