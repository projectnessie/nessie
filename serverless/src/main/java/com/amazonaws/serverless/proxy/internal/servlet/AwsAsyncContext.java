...
/**
 * Async context for Serverless Java Container. This is used to support reactive embedded servers
 * for our support for Spring Boot 2. Behind the scenes, the Async context still uses the
 * <code>CountDownLatch</code> to synchronize response generation.
 */
public class AwsAsyncContext implements AsyncContext {

  private HttpServletRequest req;
  private final HttpServletResponse res;
  private final AwsLambdaServletContainerHandler handler;
  private final List<AsyncListenerHolder> listeners;
  private long timeout;
  private final AtomicBoolean dispatched;
  private final AtomicBoolean completed;

  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.servlet.AwsAsyncContext.class);

  public AwsAsyncContext(HttpServletRequest request,
                         HttpServletResponse response,
                         AwsLambdaServletContainerHandler servletHandler) {
    log.debug(
      "Initializing async context for request: " + SecurityUtils.crlf(request.getPathInfo()) + " - "
      + SecurityUtils.crlf(request.getMethod()));
    req = request;
    res = response;
  ...
  public void dispatch() {
    try {
      ...
      handler.doFilter(req,
                       res,
                       ((AwsServletContext) req.getServletContext()).getServletForPath(req.getRequestURI()));
  ...
  /**
   * The listener holder wraps and <code>AsyncListener</code> with information about its context
   * such as the request, response, and async servlet context.
   */
