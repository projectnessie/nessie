...
public class ContainerConfig {
  ...
  private final List<String> customDomainNames;
  ...
  public static com.amazonaws.serverless.proxy.model.ContainerConfig defaultConfig() {
    com.amazonaws.serverless.proxy.model.ContainerConfig configuration = new com.amazonaws.serverless.proxy.model.ContainerConfig();
    ...
    configuration.addBinaryContentTypes("application/octet-stream",
                                        "image/jpeg",
                                        "image/png",
                                        "image/gif");
  ...
  /**
   * Returns the base path configured in the container. This configuration variable is used in
   * conjuction with {@link #setStripBasePath(boolean)} to route the request. When requesting the
   * context path from an HttpServletRequest: {@link AwsProxyHttpServletRequest#getContextPath()}
   * this base path is added to the context even though it was initially stripped for the purpose of
   * routing the request. We decided to add it to the context to address GitHub issue #84 and allow
  ...
  /**
   * Configures a base path  that can be stripped from the request path before passing it to the
   * framework-specific implementation. This can be used to remove API Gateway's base path mappings
   * from the request.
   *
  ...
  /**
   * Whether this framework should strip the base path mapping specified with the {@link
   * #setServiceBasePath(String)} method from a request before passing it to the framework-specific
   * implementations
   *
   * @param stripBasePath
   */
  ...
  /**
   * Tells the library to consolidate multiple Set-Cookie headers into a single Set-Cookie header
   * with multiple, comma-separated values. This is allowed by the RFC 2109
   * (https://tools.ietf.org/html/rfc2109). However, since not all clients support this, we consider
   * it optional. When this value is set to true the framework will consolidate all Set-Cookie
   * headers into a single header, when it's set to false, the framework will only return the first
  ...
  /**
   * Tells whether the stage name passed in the request should be added to the context path: {@link
   ...
   *
   * @param useStageAsServletContext true if you want the stage to appear as the root of the context
  ...
  /**
   * Whether query string parameters in the request should be case sensitive or not. By default this
   * is set to <code>false</code> for backward compatibility.
   *
   ...
   *
   * @param queryStringCaseSensitive Tells the framework to treat query string parameter names as
   *                                 case sensitive
   */
  ...
  /**
   * Returns the name of the default charset appended to the <code>Content-Type</code> header if no
   * charset is specified by the request. The default value of this is <code>ISO-8859-1</code>.
   *
  ...
  /**
   * Sets the default charset value for the <code>Content-Type</code> header if no charset is
   * specified with a request. The default value of this is <code>ISO-8859-1</code>. If a request
   * specifies a <code>Content-Type</code> header without a <code>charset</code> property, the value
   * of this field is automatically appended to the header.
   *
  ...
  /**
   * Returns the maximum amount of time (in milliseconds) set for the initialization time. See
   * documentation on the {@link #setInitializationTimeout(int)} for additional details.
   *
   ...
   * the underlying framework is initialized in a separate thread. Serverless Java Container will
   * wait for the maximum time available during AWS Lambda's init step (~10 seconds) and then return
   * control to the main thread. In the meanwhile, the initialization process of the underlying
   * framework can continue in a separate thread. AWS Lambda will then call the handler class to
   * handle an event. This timeout is the maximum amount of time Serverless Java Container framework
   * will wait for the underlying framework to initialize before returning an error. By default,
   * this is set to 10 seconds.
   *
   * @param initializationTimeout The maximum amount of time to wait for the underlying framework
   *                              initialization after an event is received in milliseconds.
   */
  ...
  /**
   * Whether the framework will run exception thrown by the application through the implementation
   * of {@link com.amazonaws.serverless.proxy.ExceptionHandler}. When this parameter is set to false
   * the Lambda container handler object lets the Exception propagate upwards to the Lambda handler
   * class.
   *
  ...
  /**
   * This configuration parameter tells the container whether it should skip exception mapping and
   * simply let any Exception thrown by the underlying application bubble up to the Lambda handler
   * class.
   *
   * @param disable Set this value to <code>true</code> to disable exception mapping,
