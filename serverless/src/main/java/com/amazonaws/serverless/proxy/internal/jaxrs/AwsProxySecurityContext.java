...
/**
 * default implementation of the <code>SecurityContext</code> object. This class supports 3 API
 * Gateway's authorization methods: AWS_IAM, CUSTOM_AUTHORIZER, and COGNITO_USER_POOL (oidc). The
 * Principal returned by the object depends on the authorization model: * AWS_IAM: User ARN *
 * CUSTOM_AUTHORIZER: Custom user principal * COGNITO_USER_POOL: Cognito identity id
 */
public class AwsProxySecurityContext
  ...
  private final AwsProxyRequest event;
  ...
  public Principal getUserPrincipal() {
    ...
    if (getAuthenticationScheme().equals(AUTH_SCHEME_CUSTOM) || getAuthenticationScheme().equals(
      AUTH_SCHEME_AWS_IAM)) {
      return () -> {
    ...
    switch (event.getRequestSource()) {
      case API_GATEWAY:
        if (event.getRequestContext().getAuthorizer() != null
            && event.getRequestContext().getAuthorizer().getClaims() != null
  ...
  /**
   * Custom object for request authorized with a Cognito User Pool authorizer. By casting the
   * Principal object to this you can extract the Claims object included in the token.
   */
  public static class CognitoUserPoolPrincipal implements Principal {

    private final CognitoAuthorizerClaims claims;
