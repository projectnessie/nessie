...
/**
 * Default <code>RequestDispatcher</code> implementation for the <code>AwsProxyHttpServletRequest</code>
 * type. A new instance of this object is created each time a framework gets the RequestDispatcher
 * from a servlet request. Behind the scenes, this object uses the <code>AwsLambdaServletContainerHandler</code>
...
public class AwsProxyRequestDispatcher implements RequestDispatcher {
  ...
  private final boolean isNamedDispatcher;
  private final AwsLambdaServletContainerHandler lambdaContainerHandler;
  ...
  public AwsProxyRequestDispatcher(final String target,
                                   final boolean namedDispatcher,
                                   final AwsLambdaServletContainerHandler handler) {
  ...
  public void forward(ServletRequest servletRequest, ServletResponse servletResponse)
    ...
    if (isNamedDispatcher) {
      lambdaContainerHandler.doFilter((HttpServletRequest) servletRequest,
                                      (HttpServletResponse) servletResponse,
                                      getServlet((HttpServletRequest) servletRequest));
      return;
    }
  ...
  public void include(ServletRequest servletRequest, ServletResponse servletResponse)
    ...
    if (!isNamedDispatcher) {
      ...
      servletRequest.setAttribute("javax.servlet.include.servlet_path",
                                  ((HttpServletRequest) servletRequest).getServletPath());
      servletRequest.setAttribute("javax.servlet.include.path_info",
                                  ((HttpServletRequest) servletRequest).getPathInfo());
      servletRequest.setAttribute("javax.servlet.include.query_string",
                                  SecurityUtils.encode(SecurityUtils.crlf(((HttpServletRequest) servletRequest)
                                                                            .getQueryString())));
      setRequestPath(servletRequest, dispatchTo);
    }
    lambdaContainerHandler.doFilter((HttpServletRequest) servletRequest,
  ...
  /**
   * Sets the destination path in the given request. Uses the <code>AwsProxyRequest.setPath</code>
   * method which is in turn read by the <code>HttpServletRequest</code> implementation.
   *
   ...
   *                               <code>API_GATEWAY_EVENT_PROPERTY</code> attribute or the value
   *                               for the attribute is not of the correct type:
   *                               <code>AwsProxyRequest</code>.
   */
  void setRequestPath(ServletRequest req, final String destinationPath) {
    ...
    log.debug(
      "Request is not an proxy request generated by this library, attempting to extract the proxy event type from the request attributes");
    if (req.getAttribute(API_GATEWAY_EVENT_PROPERTY) != null && req.getAttribute(
      API_GATEWAY_EVENT_PROPERTY) instanceof AwsProxyRequest) {
      ((AwsProxyRequest) req.getAttribute(API_GATEWAY_EVENT_PROPERTY)).setPath(dispatchTo);
      return;
    }
    if (req.getAttribute(HTTP_API_EVENT_PROPERTY) != null && req.getAttribute(
      HTTP_API_EVENT_PROPERTY) instanceof HttpApiV2ProxyRequest) {
      ((HttpApiV2ProxyRequest) req.getAttribute(HTTP_API_EVENT_PROPERTY)).setRawPath(destinationPath);
      return;
    }

    throw new IllegalStateException(
      "Could not set new target path for the given ServletRequest object");
  }
