...
/**
 * Basic implementation of the <code>HttpServletResponse</code> object. This is used by the
 * <code>AwsProxyHttpServletResponseWriter</code> to generate an <code>AwsProxyResponse</code>
 * object. We have an additional <code>getAwsResponseHeaders()</code> method that returns a {@code
 * Map<String, String>} that can be used for our proxy response object.
 */
public class AwsHttpServletResponse
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.servlet.AwsHttpServletResponse.class);
  ...
  /**
   * The constructor for this object receives a <code>CountDownLatch</code> to synchronize the
   * execution of the Lambda function while the response is asynchronously written by the underlying
   * container/application
   *
   * @param latch A latch used to inform the <code>ContainerHandler</code> that we are done
  ...
  public void addCookie(Cookie cookie) {
    if (request != null && request.getDispatcherType() == DispatcherType.INCLUDE && isCommitted()) {
      throw new IllegalStateException(
        "Cannot add Cookies for include request when response is committed");
    }
  ...
  public void addHeader(String s, String s1) {
    ...
    if (s.toLowerCase(Locale.getDefault())
         .equals(HttpHeaders.CONTENT_TYPE.toLowerCase(Locale.getDefault()))) {
      setContentType(s1);
    } else {
  ...
  public void setContentType(String s) {
    ...
    if (s.contains(";")) { // we have a forced charset
      setHeader(HttpHeaders.CONTENT_TYPE,
                String.format("%s; charset=%s", s.split(";")[0], characterEncoding),
                true);
    } else {
      setHeader(HttpHeaders.CONTENT_TYPE,
                String.format("%s; charset=%s", s, characterEncoding),
                true);
    }
  }
  ...
  public void flushBuffer() throws IOException {
    ...
    log.debug("Response buffer flushed with {} bytes, latch={}",
              responseBody.length(),
              writersCountDownLatch.getCount());
    isCommitted = true;
