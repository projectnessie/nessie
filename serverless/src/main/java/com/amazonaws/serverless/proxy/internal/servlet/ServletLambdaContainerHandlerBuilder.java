...
/**
 * Base builder class for {@link AwsLambdaServletContainerHandler}. Implmentations can extend this
 * class to have setters for the basic parameters.
 *
 * @param <RequestType>          The event object class
 * @param <ResponseType>         The output object class
 * @param <ContainerRequestType> The container request type. For proxy implementations, this is
 *                               {@link AwsProxyHttpServletRequest}. The response type is hardcoded
 *                               to {@link AwsHttpServletResponse} since it is a generic servlet
 *                               response implementation.
 * @param <HandlerType>          The type of the handler we are building
 * @param <Builder>              The builder object itself. This is used to allow implementations to
 *                               re-use the setter method from this abstract class through <a
 *                               href="http://www.artima.com/weblogs/viewpost.jsp?thread=133275">
...
public abstract class ServletLambdaContainerHandlerBuilder<
  ...
  Builder extends com.amazonaws.serverless.proxy.internal.servlet.ServletLambdaContainerHandlerBuilder<RequestType, ResponseType, ContainerRequestType, HandlerType, Builder>> {
   ...
   *
   * @throws ContainerInitializationException If values have not been set on the builder. The
   *                                          message in the exception contains a standard error
   *                                          message {@link com.amazonaws.serverless.proxy.internal.servlet.ServletLambdaContainerHandlerBuilder#MISSING_FIELD_ERROR}
   *                                          populated with the list of missing fields.
   */
  protected void validate() throws ContainerInitializationException {
    ...
    if (!errFields.isEmpty()) {
      throw new ContainerInitializationException(String.format(MISSING_FIELD_ERROR,
                                                               String.join(", ", errFields)), null);
    }
  }

  /**
   * Sets all of the required fields in the builder to the default settings for a Servlet-compatible
   * framework that wants to support AWS proxy event and output types.
   *
  ...
  /**
   * Sets all of the required fields in the builder to the default settings for a Servlet-compatible
   * framework that wants to support HTTP API's v2 proxy event
   *
   * @return A populated builder
   */
  ...
  /**
   * Sets the initialization wrapper to be used by the {@link com.amazonaws.serverless.proxy.internal.servlet.ServletLambdaContainerHandlerBuilder#buildAndInitialize()}
   * method to start the framework implementations
   *
   * @param initializationWrapper An implementation of <code>InitializationWrapper</code>. In most
   *                              cases, this will be set to {@link InitializationWrapper}. The
   *                              {@link com.amazonaws.serverless.proxy.internal.servlet.ServletLambdaContainerHandlerBuilder#asyncInit(long)}
   ...
   *
   * @param actualStartTime An epoch in milliseconds that should be used to calculate the 10 seconds
   *                        timeout since the start of the application
   * @return A builder configured to use the async initializer
   * @deprecated As of release 1.5 this method is deprecated in favor of the parameters-less one
   * {@link com.amazonaws.serverless.proxy.internal.servlet.ServletLambdaContainerHandlerBuilder#asyncInit()}.
   */
  @Deprecated
  ...
  /**
   * Uses a new {@link AsyncInitializationWrapper} with the no-parameter constructor that takes the
  ...
  /**
   * Implementations should implement this method to return their type. All of the builder methods
   * in this abstract class use this method to return the correct builder type.
   *
