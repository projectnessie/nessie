...
/**
 * This object receives the <code>ContainerResponse</code> instance from the Jersey application and
 * writes it to an
 * <code>AwsProxyResponse</code> object. The response object is passed in the constructor alongside
...
class JerseyServletResponseWriter
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.jersey.JerseyServletResponseWriter.class);
  private final CountDownLatch jerseyLatch;
  ...
  public OutputStream writeResponseStatusAndHeaders(long contentLength,
    ...
    for (final Map.Entry<String, List<String>> e : containerResponse.getStringHeaders()
                                                                    .entrySet()) {
