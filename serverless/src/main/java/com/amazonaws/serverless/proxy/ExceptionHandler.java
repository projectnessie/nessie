 ...
 * Normally, exceptions are handled by the client applications directly within the container and a
 * valid HTTP response is expected. This handler is used for exceptions thrown by the library while
 * marshalling and unmarshalling requests and responses.
 * <p>
 * The interface declares two methods. A typed <code>handle</code> method for requests that are
 * being proxied using a request and response type <code>LambdaContainerHandler</code>, and a
 * stream-based
 * <code>handle</code> method for <a href="http://docs.aws.amazon.com/lambda/latest/dg/java-handler-io-type-stream.html"
 * target="_blank">
 * Lambda's <code>RequestStreamHandler</code></a>.
 *
 * @param <ResponseType> The type for the Lambda return value. Implementing sub-classes are required
 *                       to return a valid instance of the response type.
...
public interface ExceptionHandler<ResponseType> {
   ...
   * <code>proxy</code> method for typed calls
   *
