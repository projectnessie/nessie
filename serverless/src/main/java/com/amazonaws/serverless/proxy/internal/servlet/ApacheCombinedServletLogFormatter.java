...
import static com.amazonaws.serverless.proxy.RequestReader.HTTP_API_CONTEXT_PROPERTY;
...
/**
 * Default implementation of the log formatter. Based on an <code>HttpServletRequest</code> and
 * <code>HttpServletResponse</code> implementations produced a log line in the Apache combined log
 * format: https://httpd.apache.org/docs/2.4/logs.html
 *
...
public class ApacheCombinedServletLogFormatter<ContainerRequestType extends HttpServletRequest, ContainerResponseType extends HttpServletResponse>
  ...
  @Override
  public String format(ContainerRequestType servletRequest,
                       ContainerResponseType servletResponse,
                       SecurityContext ctx) {
    //LogFormat "%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-agent}i\"" combined
    StringBuilder logLineBuilder = new StringBuilder();
    AwsProxyRequestContext gatewayContext = (AwsProxyRequestContext) servletRequest.getAttribute(
      API_GATEWAY_CONTEXT_PROPERTY);
    HttpApiV2ProxyRequestContext httpApiContext = (HttpApiV2ProxyRequestContext) servletRequest.getAttribute(
    ...
    if (gatewayContext != null && gatewayContext.getIdentity() != null
        && gatewayContext.getIdentity().getUserArn() != null) {
