...
public class AwsHttpApiV2SecurityContext implements SecurityContext {
  ...
  private static final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.jaxrs.AwsHttpApiV2SecurityContext.class);

  private final Context lambdaContext;
  private final HttpApiV2ProxyRequest event;
  ...
  public Principal getUserPrincipal() {
    if (getAuthenticationScheme() == null || !event.getHeaders()
    ...
    if (parts.length != 3) {
      log.warn(
        "Could not parse JWT token for requestId: " + SecurityUtils.crlf(event.getRequestContext()
                                                                              .getRequestId()));
      return null;
    }
    String decodedBody = new String(Base64.getMimeDecoder().decode(parts[1]),
    ...
    try {
      ...
      if (!parsedBody.isObject() && parsedBody.has("sub")) {
        log.debug("Could not find \"sub\" field in JWT body for requestId: " + SecurityUtils.crlf(
      ...
      log.error("Error while attempting to parse JWT body for requestId: " + SecurityUtils.crlf(
        event.getRequestContext().getRequestId()), e);
      return null;
    }
