 ...
 * <code>LambdaContainerHandler</code>.
 *
 * @param <RequestType>           The expected request object. This is the model class that the
 *                                event JSON is de-serialized to
 * @param <ResponseType>          The expected Lambda function response object. Responses from the
 *                                container will be written to this model object
 * @param <ContainerRequestType>  The request type for the wrapped Java container
 * @param <ContainerResponseType> The response or response writer type for the wrapped Java
 *                                container
 */
public abstract class LambdaContainerHandler<RequestType, ResponseType, ContainerRequestType, ContainerResponseType> {
  ...
  // Variables - Private
  //-------------------------------------------------------------
  private static final ContainerConfig config = ContainerConfig.defaultConfig();
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.internal.LambdaContainerHandler.class);
  ...
  protected abstract ContainerResponseType getContainerResponse(ContainerRequestType request,
  ...
  protected abstract void handleRequest(ContainerRequestType containerRequest,
   ...
   *
   * @return The initialization wrapper that was passed to the constructor and this instance will
   * use to decide whether it can start handling events.
   */
   ...
   *
   * @param wrapper The wrapper this instance will use to decide whether it can start handling
   *                events.
   */
  ...
  /**
   * Configures the library to strip a base path from incoming requests before passing them on to
   * the wrapped framework. This was added in response to issue #34 (https://github.com/awslabs/aws-serverless-java-container/issues/34).
   * When creating a base path mapping for custom domain names in API Gateway we want to be able to
   * strip the base path from the request - the underlying service may not recognize this path.
   *
  ...
  /**
   * Sets the formatter used to log request data in CloudWatch. By default this is set to use an
   * Apache combined log format based on the servlet request and response object {@link
  ...
  /**
   * Proxies requests to the underlying container given the incoming Lambda request. This method
   * returns a populated return object for the Lambda function.
   *
  ...
  public ResponseType proxy(RequestType request, Context context) {
    ...
    try {
      SecurityContext securityContext = securityContextWriter.writeSecurityContext(request,
                                                                                   context);
      ContainerRequestType containerRequest = requestReader.readRequest(request,
                                                                        securityContext,
                                                                        context,
                                                                        config);
      ...
      if (initializationWrapper != null && initializationWrapper.getInitializationLatch() != null) {
        ...
        if (!initializationWrapper.getInitializationLatch()
                                  .await(config.getInitializationTimeout(),
                                         TimeUnit.MILLISECONDS)) {
          throw new ContainerInitializationException(
            "Could not initialize framework within the " + config.getInitializationTimeout()
            + "ms timeout", null);
        }
      }
      ...
      if (logFormatter != null) {
        log.info(SecurityUtils.crlf(logFormatter.format(containerRequest,
                                                        containerResponse,
                                                        securityContext)));
      }
  ...
  /**
   * Handles Lambda <code>RequestStreamHandler</code> method. The method uses an
   * <code>ObjectMapper</code> to transform the incoming input stream into the given {@link
   * RequestType} and then calls the {@link #proxy(Object, Context)} method to handle the request.
   * The output from the proxy method is written on the given output stream.
   *
