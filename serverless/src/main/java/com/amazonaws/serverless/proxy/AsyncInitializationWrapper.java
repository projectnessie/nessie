...
/**
 * An async implementation of the <code>InitializationWrapper</code> interface. This initializer
 * calls the {@link LambdaContainerHandler#initialize()} in a separate thread. Then uses a latch to
 * wait for the maximum Lambda initialization time of 10 seconds, if the <code>initialize</code>
 * method takes longer than 10 seconds to return, the {@link #start(LambdaContainerHandler)} returns
 * control to the caller and lets the initialization thread continue in the background. The {@link
 * LambdaContainerHandler#proxy(Object, Context)} automatically waits for the latch of the
 * initializer to be released.
 * <p>
 * The constructor of this class expects an epoch long. This is meant to be as close as possible to
 * the time the Lambda function actually started. In most cases, the first action in the constructor
 * of the handler class should be to populate this long value ({@code Instant.now().toEpochMs();}).
...
public class AsyncInitializationWrapper extends InitializationWrapper {
  ...
  private final Logger log = LoggerFactory.getLogger(com.amazonaws.serverless.proxy.AsyncInitializationWrapper.class);

  /**
   * Creates a new instance of the async initializer.
   *
   * @param startTime The epoch ms start time of the Lambda function, this should be measured as
   *                  close as possible to the initialization of the function.
   */
  ...
  /**
   * Creates a new instance of the async initializer using the actual JVM start time as the starting
   * point to measure the 10 seconds timeout.
   */
  ...
  public void start(LambdaContainerHandler handler) throws ContainerInitializationException {
    ...
    try {
      ...
      if (!initializationLatch.await(awaitTime, TimeUnit.MILLISECONDS)) {
        log.info("Initialization took longer than " + LAMBDA_MAX_INIT_TIME_MS
                 + ", setting new CountDownLatch and " +
  ...
  private static class AsyncInitializer implements Runnable {
    ...
    private final LambdaContainerHandler handler;
