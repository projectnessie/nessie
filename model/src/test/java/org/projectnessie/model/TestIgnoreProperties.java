...
class TestIgnoreProperties {
  ...
  void multiGet() throws Exception {
    doTestUnknown(MultiGetContentsRequest.of(ContentsKey.of("foo", "bar")), MultiGetContentsRequest.class);
    doTestUnknown(MultiGetContentsResponse.of(Collections.singletonList(
        ContentsWithKey.of(ContentsKey.of("foo", "bar"), IcebergTable.of("here/there")))), MultiGetContentsResponse.class);
  }

  @Test
  void paginated() throws Exception {
    doTestUnknown(EntriesResponse.builder()
        .addEntries(Entry.builder().name(ContentsKey.of("entry-name")).type(Type.ICEBERG_TABLE).build())
        .build(), EntriesResponse.class);
    doTestUnknown(ImmutableLogResponse.builder()
        .addOperations(CommitMeta.builder()
  ...
  void operations() throws Exception {
    Operations ops = ImmutableOperations.builder()
        ...
        .addOperations(Put.of(ContentsKey.of("put", "iceberg"), IcebergTable.of("here/and/there")))
        .addOperations(Put.of(ContentsKey.of("put", "delta"), ImmutableDeltaLakeTable.builder()
            .lastCheckpoint("checkpoint")
            .addMetadataLocationHistory("meta", "data", "location")
            .addCheckpointLocationHistory("checkpoint")
            .id("delta-id").build()))
        .addOperations(Put.of(ContentsKey.of("put", "hive"), ImmutableHiveTable.builder()
  ...
  private <T> void doTestUnknown(T value, Class<T> type) throws Exception {
    ...
    JsonGenerator mock = new JsonGeneratorDelegate(jsonGen) {
      @Override
