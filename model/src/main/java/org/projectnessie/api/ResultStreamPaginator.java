...
final class ResultStreamPaginator<R extends PaginatedResponse, E> {
  ...
  interface Fetcher<R> {
     ...
     */
    R fetch(@NotNull String ref, Integer pageSize, String token)
  ...
  ResultStreamPaginator(Function<R, List<E>> entriesFromResponse,
      Fetcher<R> fetcher) {
   ...
   * <p>This implementation fetches the first page eagerly to propagate {@link NessieNotFoundException}.</p>
   * @param ref reference
   ...
   */
  Stream<E> generateStream(@NotNull String ref, OptionalInt pageSizeHint) throws NessieNotFoundException {
    R firstPage = fetcher.fetch(ref, pageSizeHint.isPresent() ? pageSizeHint.getAsInt() : null, null);
    Spliterator<E> spliterator = new Spliterator<E>() {
      ...
      public boolean tryAdvance(Consumer<? super E> action) {
        ...
        if (currentPage != null && entriesFromResponse.apply(currentPage).size() == offsetInPage) {
