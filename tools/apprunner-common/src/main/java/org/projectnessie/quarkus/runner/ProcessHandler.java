 ...
 * <p>Starts the process configured in a {@link ProcessBuilder}, provides a method to get the
 * {@link #getListenUrl() Quarkus HTTP listen URL} as Quarkus prints to stdout, and manages process
 * lifetime and line-by-line I/O pass-through for stdout + stderr.</p>
 * <p>Any instance of this class can only be used to start (and stop) one process and
 * cannot be reused for another process.</p>
 */
public class ProcessHandler {
   ...
   * <p>The name will be used to name the background I/O handling thread.</p>
   */
   ...
   * <p>If the Quarkus process does not emit that URL within the time configured via
   * {@link #setTimeToListenUrlMillis(long)}, which defaults to {@value #MILLIS_TO_HTTP_PORT} ms,
   * this method will throw an {@link IllegalStateException}.</p>
   * @return the listen URL, never {@code null}.
   * @throws InterruptedException if the current thread was interrupted while waiting for the listen URL.
   ...
   * after {@link #setTimeStopMillis(long)}, which defaults to {@value #MILLIS_TO_STOP} ms,
   * the process will be killed with a {@code SIGKILL}.</p>
   */
  public void stop() {
    if (process == null) {
   ...
   * {@link IllegalThreadStateException} if it is still alive.
   *
