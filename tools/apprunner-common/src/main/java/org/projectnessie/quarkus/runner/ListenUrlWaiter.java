...
/**
 * Accepts {@link String}s via it's {@link #accept(String)} method and checks for the
 * {@code Listening on: http...} pattern.
 */
final class ListenUrlWaiter implements Consumer<String> {
  ...
  String getListenUrl() throws InterruptedException, TimeoutException {
    while (true) {
      ...
      // must succeed if the listen-url has been captured, even if it's called after the timeout has elapsed
      ...
      try {
        ...
        // This "short get()" is implemented to make the unit test TestListenUrlWaiter.noTimeout() run faster.
