...
quarkus {
  ...
  // Override db-kind only when set (e.g. -Pnessie.server-admin-tool.datasource.db-kind=mssql)
  project.findProperty("nessie.server-admin-tool.datasource.db-kind")?.toString()?.let { dbKind ->
    quarkusBuildProperties.put("quarkus.datasource.db-kind", dbKind)
  }
