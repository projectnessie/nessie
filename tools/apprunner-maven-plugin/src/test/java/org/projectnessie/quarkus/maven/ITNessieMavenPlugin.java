...
class ITNessieMavenPlugin {
  ...
  void executionJarAndApplicationIdMissing(MavenExecutionResult result) {
    assertThat(result).isFailure().out().error()
        .anyMatch(s -> s.contains("Either appArtifactId or executableJar config option must be specified, prefer appArtifactId"));
  }
  ...
  void executionJarAndApplicationIdSpecified(MavenExecutionResult result) {
    assertThat(result).isFailure().out().error()
        .anyMatch(s -> s.contains("The options appArtifactId and executableJar are mutually exclusive"));
  }
  ...
  void applicationIdSpecified(MavenExecutionResult result) {
    assertThat(result).isSuccessful().out().info()
        .anyMatch(s -> s.matches("Starting process: .*-jar .*/nessie-quarkus-.*-runner.jar, additional env: .*HELLO=world.*"))
