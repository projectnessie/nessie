...
/**
 * A tool for canonicalizing an expression before sending to DynamoDb. Responsible for replacing
 * path names and values with aliases where needed. Should be used once per expression.
 */
public class AliasCollectorImpl implements AliasCollector {
   ...
   *
   * <p>If the string is escaped, the mapping is recorded in the collectors list of attribute names
   * for later use.
   *
  ...
  public String escape(String unescaped) {
    // If an attribute name begins with a number or contains a space, a special character, or a
    // reserved word,
