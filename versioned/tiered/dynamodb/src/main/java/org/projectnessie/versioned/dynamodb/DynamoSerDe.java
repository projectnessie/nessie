...
final class DynamoSerDe {
  ...
  private static final Map<ValueType<?>, Supplier<DynamoBaseValue<?>>> ENTITY_MAP_PRODUCERS =
      ImmutableMap.<ValueType<?>, Supplier<DynamoBaseValue<?>>>builder()
          ...
          .build();
  private static final Map<ValueType<?>, BiConsumer<Map<String, AttributeValue>, BaseValue<?>>>
      DESERIALIZERS =
          ImmutableMap
              .<ValueType<?>, BiConsumer<Map<String, AttributeValue>, BaseValue<?>>>builder()
  ...
  static {
    ...
    if (!ENTITY_MAP_PRODUCERS.keySet().containsAll(ValueType.values())) {
      throw new UnsupportedOperationException(
          String.format(
              "The implementation of the Dynamo backend does not have "
              ...
              ENTITY_MAP_PRODUCERS.keySet(), ValueType.values()));
    }
  }
   ...
   *
   * <p>The actual entity to serialize is not passed into this method, but this method calls a Java
   * {@link Consumer} that receives an instance of {@link DynamoBaseValue} that receives the entity
   * components.
   */
  ...
  /**
   * Deserialize the given {@code entity} as the given {@link ValueType type} directly into the
   * given {@code consumer}.
   *
  ...
  public static <C extends BaseValue<C>> void deserializeToConsumer(
    ...
    String loadedType =
        Preconditions.checkNotNull(
            attributeValue(entity, ValueType.SCHEMA_TYPE).s(),
    ...
    Preconditions.checkArgument(
        valueType.getValueName().equals(loadedType),
        "Expected schema for id %s to be of type '%s' but is actually '%s'.",
        id.getHash(),
        valueType.name(),
        loadedType);

    // No need for any 'valueType' validation against the static map - that's done in the static
    // initializer
