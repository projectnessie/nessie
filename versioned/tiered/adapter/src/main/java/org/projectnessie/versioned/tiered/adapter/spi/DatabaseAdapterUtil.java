...
public abstract class DatabaseAdapterUtil implements DatabaseAdapter {
  ...
  protected static String mergeConflictMessage(
    ...
    return String.format(
        "%s during merge of %s%s into %s%s %s requiring a common ancestor",
        err,
        from.getName(),
  ...
  protected static String commitConflictMessage(
      String err, BranchName branch, Optional<Hash> expectedHead, Set<Key> operationsKeys) {
    return String.format(
        "%s during commit against '%s%s' with keys %s",
        err,
        branch.getName(),
        expectedHead.map(h -> "@" + h.asString()).orElse(""),
        operationsKeys.stream()
            .map(k -> '\'' + k.toString() + '\'')
