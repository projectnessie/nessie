...
public class DbObjectsSerializers extends Serializers.Base {
  ...
  private static final JsonSerializer<Hash> HASH_KEY_SERIALIZER =
      new JsonSerializer<Hash>() {
        @Override
        public void serialize(Hash value, JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
          gen.writeFieldName("h" + value.asString());
        }
      };

  private static final KeyDeserializer HASH_KEY_DESERIALIZER =
      new KeyDeserializer() {
        @Override
        public Object deserializeKey(String key, DeserializationContext ctxt) {
          return Hash.of(key.substring(1));
        }
      };
  ...
  private static final JsonSerializer<NamedRef> NAMED_REF_KEY_SERIALIZER =
      new JsonSerializer<NamedRef>() {
        @Override
        public void serialize(NamedRef value, JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
          if (value instanceof TagName) {
            gen.writeFieldName("t_" + value.getName());
          } else if (value instanceof BranchName) {
            gen.writeFieldName("b_" + value.getName());
          }
        }
      };

  private static final KeyDeserializer NAMED_REF_KEY_DESERIALIZER =
      new KeyDeserializer() {
        @Override
        public Object deserializeKey(String key, DeserializationContext ctxt) {
          if (key.startsWith("b_")) {
            return BranchName.of(key.substring(2));
          } else if (key.startsWith("t_")) {
            return TagName.of(key.substring(2));
          }
          return null;
        }
      };
  ...
  private static final JsonSerializer<Key> KEY_KEY_SERIALIZER =
      new JsonSerializer<Key>() {
        @Override
        public void serialize(Key value, JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
          gen.writeFieldName(flattenKey(value));
        }
      };

  private static final KeyDeserializer KEY_KEY_DESERIALIZER =
      new KeyDeserializer() {
        @Override
        public Object deserializeKey(String key, DeserializationContext ctxt) {
          return fromFlattened(key);
        }
      };
  ...
  public static ObjectMapper register(ObjectMapper mapper) {
    ...
    KeyDeserializers keyDeserializers =
        (type, config, beanDesc) -> {
          if (type.isTypeOrSubTypeOf(Hash.class)) {
            return HASH_KEY_DESERIALIZER;
          }
          if (type.isTypeOrSubTypeOf(NamedRef.class)) {
            return NAMED_REF_KEY_DESERIALIZER;
          }
          if (type.isTypeOrSubTypeOf(Key.class)) {
            return KEY_KEY_DESERIALIZER;
          }
          return null;
        };

    Serializers keySerializers =
        new Serializers.Base() {
          @Override
          public JsonSerializer<?> findSerializer(
              SerializationConfig config, JavaType type, BeanDescription beanDesc) {
            if (type.isTypeOrSubTypeOf(Hash.class)) {
              return HASH_KEY_SERIALIZER;
            }
            if (type.isTypeOrSubTypeOf(NamedRef.class)) {
              return NAMED_REF_KEY_SERIALIZER;
            }
            if (type.isTypeOrSubTypeOf(Key.class)) {
              return KEY_KEY_SERIALIZER;
            }
            return null;
          }
        };
    ...
    module.addKeyDeserializer(NamedRef.class, NAMED_REF_KEY_DESERIALIZER);
