...
public interface KeyWithType {
  ...
  @Value.Auxiliary
  // 'type' is auxiliary to exclude 'type' from equals + hashCode calculations, which makes key-list
  // construction much easier.
  byte getType();

  static KeyWithType of(Key key, byte type) {
    return ImmutableKeyWithType.builder().key(key).type(type).build();
  }
}
