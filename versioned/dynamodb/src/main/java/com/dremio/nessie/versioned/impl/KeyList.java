...
abstract class KeyList {
  ...
  public static final KeyList EMPTY = new CompleteList(Collections.emptyList());
  ...
  abstract static class IncrementalList extends KeyList {
    ...
    private CompleteList generateNewCheckpoint(L1 startingPoint, DynamoStore store) {
      ...
      return accum.getCompleteList();
    }

    private IterResult getKeysIter(L1 startingPoint, DynamoStore store) {
      HistoryRetriever retriever = new HistoryRetriever(store, startingPoint, getPreviousCheckpoint());
  ...
  static class CompleteList extends KeyList {
    ...
    public CompleteList(List<Id> fragmentIds) {
    ...
    public AttributeValue toAttributeValue() {
      return AttributeValue.builder().m(ImmutableMap.<String, AttributeValue>of(
            ...
            AttributeValue.builder()
                .l(fragmentIds.stream().map(Id::toAttributeValue).collect(Collectors.toList()))
                .build()
                )
            ).build();
    }
    ...
    static KeyList fromAttributeValue(Map<String, AttributeValue> value) {
      return new CompleteList(value.get(FRAGMENTS).l().stream().map(Id::fromAttributeValue).collect(Collectors.toList()));
    }
  ...
  static class KeyAccumulator {
    ...
    public CompleteList getCompleteList() {
      Preconditions.checkArgument(currentList.isEmpty());
      return new CompleteList(fragmentIds);
    }
  }
