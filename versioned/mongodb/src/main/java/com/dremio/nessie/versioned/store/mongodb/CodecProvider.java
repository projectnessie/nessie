...
class CodecProvider implements org.bson.codecs.configuration.CodecProvider {
  ...
  static class EntityToBsonConverter {
    ...
    /**
     * Writes a single Entity to BSON. Invokes different write methods based on the underlying
     * {@code com.dremio.nessie.versioned.store.Entity} type.
     *
     * @param writer the BSON writer to write to.
      ...
      switch (value.getType()) {
        ...
        default:
          throw new UnsupportedOperationException(
              String.format("Unsupported field type: %s", value.getType().name()));
      }
    }
  }
        ...
        switch (reader.getCurrentBsonType()) {
          ...
          case BINARY:
            mapBuilder.put(
                name,
                Entity.ofBinary(
                    UnsafeByteOperations.unsafeWrap(reader.readBinaryData().getData())));
            break;
          default:
            throw new UnsupportedOperationException(
  ...
  private static class EntityCodec<C> implements Codec<C> {
    ...
    /**
     * This serializes an object into a BSON stream. The serialization is delegated to {@link
  ...
  static {
    ...
    Arrays.stream(ValueType.values())
        .forEach(
            v ->
                builder.put(
                    v.getObjectClass(), new EntityCodec<>(v.getObjectClass(), v.getSchema())));

    // Specific case where the ID is not encoded as a document, but directly as a binary value. Keep
    // within this provider
    // as Mongo appears to rely on the same provider for all related codecs, and splitting this to a
    // separate provider
  ...
  public <T> Codec<T> get(Class<T> clazz, CodecRegistry registry) {
    ...
    // In most cases, the codec for a class is directly entered, but also account for when there are
    // subclasses for
