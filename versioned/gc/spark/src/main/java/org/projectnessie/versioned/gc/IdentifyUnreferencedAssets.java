...
public class IdentifyUnreferencedAssets<T, R extends AssetKey> {
  ...
  private static <T, R extends AssetKey> Dataset<UnreferencedItem> go(
      Serializer<T> valueSerializer,
    ...
    AssetFlatMapper<T, R> mapper =
        new AssetFlatMapper<T, R>(valueSerializer, assetKeySerializer, assetKeyConverter);
    Dataset<CategorizedAssetKey> assets =
        categorizedValues
            .filter(valueTypeFilter)
            .flatMap(mapper, Encoders.bean(CategorizedAssetKey.class));

    // generate a bloom filter of referenced items.
    final BinaryBloomFilter referencedAssets =
        BinaryBloomFilter.aggregate(
            assets.filter("referenced = true").select("uniqueKey"), "uniqueKey");
    // generate list of maybe referenced assets (note that a single asset may be referenced by both
    // referenced and non-referenced values).
    // TODO: convert this to a group by asset, date and then figure out the latest date so we can
    // include that
    // in the written file to avoid stale -> not stale -> stale values.
    Dataset<Row> unreferencedAssets =
        assets
            .filter("referenced = false")
            .select("data", "timestamp", "uniqueKey", "key")
            .filter(new AssetFilter(referencedAssets))
            .dropDuplicates("uniqueKey");
  ...
  /**
   * Spark filter to determine if a value is referenced by checking if the byte[] serialization is
   * in a bloom filter.
   */
  ...
  public static class UnreferencedItemConverter
      implements Function1<Row, UnreferencedItem>, Serializable {
  ...
  /**
   * Unreferenced Item. Pair of name of unreferenced item and the byte[] representation of the
   * underlying AssetKey.
   */
  ...
  /**
   * Spark flat map function to convert a value into an iterator of AssetKeys keeping their
   * reference state.
   */
  public static class AssetFlatMapper<T, R extends AssetKey>
     ...
     */
    public AssetFlatMapper(
        Serializer<T> valueWorker,
        Serializer<AssetKey> assetKeySerializer,
        AssetKeyConverter<T, R> assetKeyConverter) {
    ...
    public Iterator<CategorizedAssetKey> call(CategorizedValue r) throws Exception {
      ...
      return assetKeyConverter
          .apply(contents)
          .map(
              ak ->
                  new CategorizedAssetKey(
                      r.isReferenced(),
                      assetKeySerializer.toBytes(ak),
                      ak.toUniqueKey(),
                      r.getTimestamp(),
                      r.getKey()))
          .iterator();
