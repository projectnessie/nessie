...
class ITNessieSpark3 extends AbstractSparkTest {
  ...
  void testSQL() {
    ...
    String tableId2 = "nessie.delta_table";
    spark.sql(String.format("CREATE TABLE %s (id BIGINT NOT NULL, data STRING) USING delta LOCATION %s", tableId2,
                            deltaLocalDir.getAbsolutePath()));
    spark.sql(String.format("INSERT INTO %s VALUES (1, 'a'), (2, 'b'), (3, 'c')", tableId2));

    Dataset<Row> table1 = spark.sql("select * from " + tableId);
    Dataset<Row> table2 = spark.sql("select * from " + tableId2);
    ...
    spark.sql("DROP TABLE IF EXISTS " + tableId2);
  }
}
