...
public class NessieCatalog extends BaseMetastoreCatalog implements AutoCloseable {
    ...
    try {
      ...
      if (requestedRef != null) {
        throw new IllegalArgumentException(
            String.format(
                "Nessie ref '%s' provided via %s does not exist. "
                    + "This ref must exist before creating a NessieCatalog.",
                requestedRef, CONF_NESSIE_REF),
            ex);
      }
  ...
  private IcebergTable table(TableIdentifier tableIdentifier) {
    try {
      Contents table =
          client.getContentsApi().getContents(toKey(tableIdentifier), reference.getHash());
  ...
  protected TableOperations newTableOps(TableIdentifier tableIdentifier) {
    ParsedTableIdentifier pti =
        ParsedTableIdentifier.getParsedTableIdentifier(tableIdentifier, ImmutableMap.of());
    UpdateableReference newReference = this.reference;
    if (pti.getReference() != null) {
      newReference = get(pti.getReference());
    }
    return new NessieTableOperations(config, toKey(pti.getTableIdentifier()), newReference, client);
  }
  ...
  public List<TableIdentifier> listTables(Namespace namespace) {
    try {
      return client.getTreeApi().getEntries(reference.getHash()).getEntries().stream()
  ...
  private static Predicate<EntriesResponse.Entry> namespacePredicate(Namespace ns) {
    ...
    Predicate<EntriesResponse.Entry> predicate =
        e -> {
  ...
  public void renameTable(TableIdentifier from, TableIdentifier to) {
    ...
    Operations c =
        ImmutableOperations.builder()
            .addOperations(
                ImmutablePut.builder().key(toKey(to)).contents(existingFromTable).build())
    ...
    try {
      client
          ...
          .commitMultipleOperations(
              reference.getAsBranch().getName(), reference.getHash(), "iceberg rename table", c);
      // TODO: fix this so we don't depend on it in tests.
      refresh();
    } catch (Exception e) {
