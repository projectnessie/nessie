...
import com.dremio.nessie.model.{
  ContentsKey,
  DeltaLakeTable,
...
class NessieLogStore(sparkConf: SparkConf, hadoopConf: Configuration)
  ...
  private val client: NessieClient = {
    val authType = AuthType.valueOf(
      hadoopConf.get(CONF_NESSIE_AUTH_TYPE, NESSIE_AUTH_TYPE_DEFAULT)
    )
        ...
        if (requestedRef != null)
          throw new IllegalArgumentException(
            s"Nessie ref $requestedRef provided " +
              s"via $CONF_NESSIE_REF does not exist. This ref must exist before creating a NessieCatalog.",
            ex
          )
        throw new IllegalArgumentException(
          s"Nessie does not have an existing default branch. Either configure " +
            s"an alternative ref via $CONF_NESSIE_REF or create the default branch on the server.",
          ex
        )
    }
  }
  ...
  private def commit(
    ...
    val messageWithSparkId =
      s"$message ; spark.app.id=${sparkConf.get("spark.app.id")}"
    val table = updateDeltaTable(path, targetRef, lastCheckpoint)
    client.getContentsApi.setContents(
      pathToKey(path.getParent),
      targetRef,
      targetHash,
      messageWithSparkId,
      table
