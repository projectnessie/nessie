  ...
  public void setConf(Configuration conf) {
    ...
    if (hasDelegate) {
      ...
      if (nessieDbs.isEmpty()) {
        logger.warn(
            "Using delegating Nessie store but no databases were routed to use Nessie functionality. "
                + "Please update the {} configuration property in your Hive configuration.",
            NessieStore.NESSIE_WHITELIST_DBS_OPTION);
      } else {
        logger.debug(
            "Configuring a delegating store where the following databases are considered Nessie databases: {}.",
            nessieDbs);
      }
